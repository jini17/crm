...
class Outlook {
    ...
    function vtlib_handler($moduleName, $eventType) {
        ...
        $syncModules = array('Contacts' => 'Outlook Contacts', 'Calendar' => 'Outlook Calendar');
        ...
        if ($eventType == 'module.postinstall') {
			...
			$adb->pquery('UPDATE vtiger_settings_field SET active=1 WHERE name=?',array($this->LBL_OUTLOOK));
        } else if ($eventType == 'module.enabled') {
            $this->addMapWidget($forModules);
            $this->addWidgetforSync($syncModules);
			$adb->pquery('UPDATE vtiger_settings_field SET active=0 WHERE name=?',array($this->LBL_OUTLOOK));
    ...
    function addMapWidget($moduleNames, $widgetType = 'DETAILVIEWSIDEBARWIDGET', $widgetName = 'Google Map') {
        ...
        foreach ($moduleNames as $moduleName) {
            ...
            if ($module) {
                $module->addLink($widgetType, $widgetName, 'module=Outlook&view=Map&mode=showMap&viewtype=detail', '', '', '');
            }
        }
    }
    ...
    function removeMapWidget($moduleNames, $widgetType = 'DETAILVIEWSIDEBARWIDGET', $widgetName = 'Google Map') {
        ...
        foreach ($moduleNames as $moduleName) {
            $module = Vtiger_Module::getInstance($moduleName);
            if ($module) {
                $module->deleteLink($widgetType, $widgetName, 'module=Outlook&view=Map&mode=showMap&viewtype=detail');
            }
        }
    }
    ...
    function addWidgetforSync($moduleNames, $widgetType = 'LISTVIEWSIDEBARWIDGET') {
        ...
        foreach ($moduleNames as $moduleName => $widgetName) {
            $module = Vtiger_Module::getInstance($moduleName);
            if ($module) {
                $module->addLink($widgetType, $widgetName, "module=Outlook&view=List&sourcemodule=$moduleName", '', '', '');
            }
        }
    }
