...
Class Outlook_Contacts_Connector extends WSAPP_TargetConnector {
	...
	/**
	 * Get the name of the Outlook Connector
	 * @return string
	 */
	 ...
	 * @param <Outlook_Contacts_Model> $user
	 * @return <Mixed>
	 */
	public function getOutlookFieldValue($googleFieldDetails, $googleRecord, $user) {
		$googleFieldValue = '';
	...
	/**
	 * Tarsform Outlook Records to Vtiger Records
	 * @param <array> $targetRecords 
	 * @return <array> tranformed Outlook Records
	 */
	public function transformToSourceRecord($targetRecords, $user = false) {
		...
		if(!isset($this->fieldMapping)) {
			$this->fieldMapping = Outlook_Utils_Helper::getFieldMappingForUser($user);
		}
		...
		foreach ($targetRecords as $googleRecord) {
			if ($googleRecord->getMode() != WSAPP_SyncRecordModel::WSAPP_DELETE_MODE) {
				...
				foreach($this->fieldMapping as $vtFieldName => $googleFieldDetails) {
					$googleFieldValue = $this->getOutlookFieldValue($googleFieldDetails, $googleRecord, $user);
				...
				if (empty($entity['lastname'])) {
					if (!empty($entity['firstname'])) {
						...
						$entity['lastname'] = 'Outlook Contact';
					} else {
						continue;
					}
			   }
	...
	function fetchContactsFeed($query) {
		...
		$headers = array(
			'Authorization: Bearer ' => $this->apiConnection->token['access_token'],
	...
	public function getContacts($SyncState, $user = false) {
		...
		if(!isset($this->selectedGroup))
			$this->selectedGroup = Outlook_Utils_Helper::getSelectedContactGroupForUser($user);
		...
		if (Outlook_Utils_Helper::getSyncTime('Contacts', $user)) {
			$query['updated-min'] = $this->googleFormat(Outlook_Utils_Helper::getSyncTime('Contacts', $user));
		...
		$this->totalRecords =  $this->fireRequest(self::CONTACTS_URI_COUNT, $headers, $query, 'GET');;
		$contactRecords = array();
		if (count($feed->value) > 0) {
			$lastEntry = end($feed->value);
			$maxModifiedTime = date('Y-m-d H:i:s', strtotime(Outlook_Contacts_Model::vtigerFormat($lastEntry->LastModifiedDateTime)) + 1);
			if ($this->totalRecords > $this->maxResults) {
				if (!Outlook_Utils_Helper::getSyncTime('Contacts', $user)) {
					$query['updated-min'] = $this->googleFormat(date('Y-m-d H:i:s', strtotime(Outlook_Contacts_Model::vtigerFormat($lastEntry['updated']['$t']))));
		...
		foreach ($contactRecords as $i => $contact) {
			$recordModel = Outlook_Contacts_Model::getInstanceFromValues(array('entity' => $contact));
			$deleted = false;
		...
		if (isset($maxModifiedTime)) {
			Outlook_Utils_Helper::updateSyncTime('Contacts', $maxModifiedTime, $user);
		} else {
			Outlook_Utils_Helper::updateSyncTime('Contacts', false, $user);
		}
	 ...
	 * @param <Outlook_Contacts_Model> $entity
	 * @param <Users_Record_Model> $user
	 */
	protected function addEntityDetailsToAtomEntry(&$entry,$entity,$user) {
		$gdNS = $this->NS['gd'];
		$gdName = $entry->addChild("name",'',$gdNS);
		if($entity->get('salutationtype')) $gdName->addChild("namePrefix", Outlook_Utils_Helper::toOutlookXml($entity->get('salutationtype')),$gdNS);
		if($entity->get('firstname')) $gdName->addChild("givenName",  Outlook_Utils_Helper::toOutlookXml($entity->get('firstname')),$gdNS);
		if($entity->get('lastname')) $gdName->addChild("familyName", Outlook_Utils_Helper::toOutlookXml($entity->get('lastname')),$gdNS);        
		$gdRel = $gdNS . '#';
		...
		if($entity->get('account_id') || $entity->get('title')) {
			...
			if($entity->get('account_id')) $gdOrganization->addChild("orgName",  Outlook_Utils_Helper::toOutlookXml($entity->get('account_id')),$gdNS);
			if($entity->get('title')) $gdOrganization->addChild("orgTitle", Outlook_Utils_Helper::toOutlookXml($entity->get('title')),$gdNS);
		}
			...
			switch ($googleFieldDetails['google_field_name']) {  
				case 'gd:email' : 
					if($entity->get($vtFieldName)) {
						...
						$gdEmail->addAttribute("address", Outlook_Utils_Helper::toOutlookXml($entity->get($vtFieldName)));
				...
				case 'gd:phoneNumber' :
					if($entity->get($vtFieldName)) {
						$gdPhoneMobile = $entry->addChild("phoneNumber",Outlook_Utils_Helper::toOutlookXml($entity->get($vtFieldName)),$gdNS);
						if($googleFieldDetails['google_field_type'] == 'custom')
				...
				case 'gd:structuredPostalAddress' : 
					if($vtFieldName == 'mailingaddress') {
						if($entity->get('mailingstreet') || $entity->get('mailingpobox') || $entity->get('mailingzip') ||
							...
							if($entity->get('mailingstreet')) $gdAddressHome->addChild("street", Outlook_Utils_Helper::toOutlookXml($entity->get('mailingstreet')),$gdNS);
							if($entity->get('mailingpobox')) $gdAddressHome->addChild("pobox",  Outlook_Utils_Helper::toOutlookXml($entity->get('mailingpobox')),$gdNS);
							if($entity->get('mailingzip')) $gdAddressHome->addChild("postcode",  Outlook_Utils_Helper::toOutlookXml($entity->get('mailingzip')),$gdNS);
							if($entity->get('mailingcity')) $gdAddressHome->addChild("city",  Outlook_Utils_Helper::toOutlookXml($entity->get('mailingcity')),$gdNS);
							if($entity->get('mailingstate')) $gdAddressHome->addChild("region", Outlook_Utils_Helper::toOutlookXml($entity->get('mailingstate')),$gdNS);
							if($entity->get('mailingcountry')) $gdAddressHome->addChild("country", Outlook_Utils_Helper::toOutlookXml($entity->get('mailingcountry')),$gdNS);
						}
					} else {
						if($entity->get('otherstreet') || $entity->get('otherpobox') || $entity->get('otherzip') ||
							...
							if($entity->get('otherstreet')) $gdAddressWork->addChild("street", Outlook_Utils_Helper::toOutlookXml($entity->get('otherstreet')),$gdNS);
							if($entity->get('otherpobox')) $gdAddressWork->addChild("pobox", Outlook_Utils_Helper::toOutlookXml($entity->get('otherpobox')),$gdNS);
							if($entity->get('otherzip')) $gdAddressWork->addChild("postcode", Outlook_Utils_Helper::toOutlookXml($entity->get('otherzip')),$gdNS);
							if($entity->get('othercity')) $gdAddressWork->addChild("city", Outlook_Utils_Helper::toOutlookXml($entity->get('othercity')),$gdNS);
							if($entity->get('otherstate')) $gdAddressWork->addChild("region", Outlook_Utils_Helper::toOutlookXml($entity->get('otherstate')),$gdNS);
							if($entity->get('othercountry')) $gdAddressWork->addChild("country", Outlook_Utils_Helper::toOutlookXml($entity->get('othercountry')),$gdNS);
						}
					}
					break;
				case 'content' : 
					if($entity->get($vtFieldName)) $entry->addChild('content', Outlook_Utils_Helper::toOutlookXml($entity->get($vtFieldName)));
					break;
				case 'gContact:userDefinedField' : 
					if($entity->get($vtFieldName) && $googleFieldDetails['google_custom_label']) {
						...
						$userDefinedField->addAttribute('value', Outlook_Utils_Helper::toOutlookXml($this->mbEncode($entity->get($vtFieldName))));
					}
					break;
				case 'gContact:website' : 
					if($entity->get($vtFieldName)) {
						...
						$websiteField->addAttribute('href', Outlook_Utils_Helper::toOutlookXml($this->mbEncode($entity->get($vtFieldName))));
					}
					break;
			}
		}
	...
	/**
	 * Function to get OutlookContacts-ContactsGroup map for the supplied records
	 * @global  $default_charset
	 * @param <Array> $records
	 * @param <Users_Record_Model> $user
	 * @return <Array>
	...
	protected function pushChunk($records,$user) {
		...
		i/*f($records[0]->getMode() == WSAPP_SyncRecordModel::WSAPP_UPDATE_MODE) {
	 ...
	 * @param <Array Outlook_Contacts_Model> $records
	 * @param <Users_Record_Model> $user
	...
	/**
	 * Tarsform  Vtiger Records to Outlook Records
	...
	public function transformToTargetRecord($vtContacts) {
		...
		foreach ($vtContacts as $vtContact) {
			$recordModel = Outlook_Contacts_Model::getInstanceFromValues(array('entity' => $vtContact));
