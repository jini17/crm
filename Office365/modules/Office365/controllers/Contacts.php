...
class Outlook_Contacts_Controller extends WSAPP_SynchronizeController {
    ...
    public function getTargetConnector() {
        $oauth2Connector = new Outlook_Oauth2_Connector($this->getSourceType(),$this->user->id); 
        $oauth2Connection = $oauth2Connector->authorize(); 
        $connector = new Outlook_Contacts_Connector($oauth2Connection); 
        $connector->setSynchronizeController($this);
        return $connector;
    }
    
    public function getSourceConnector() { 
        $connector = new Outlook_Vtiger_Connector(); 
        $connector->setSynchronizeController($this); 
