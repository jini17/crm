...
class Google_Calendar_Model extends WSAPP_SyncRecordModel {
    ...
    public function getModifiedTime() {
        //return $this->vtigerFormat($this->value[0]->LastModifiedDateTime->getUpdated());

		 $arr=$this->value;
       	 $lastModifiedDateTime=array();
         if(is_array($arr)){
            foreach ($arr as $lastModifiedDateTime) {
                $lastModifiedDateTime[]=$lastModifiedDateTime->LastModifiedDateTime;
                
                //print_r($lastModifiedDateTime);
            }
        }
        else
            $lastModifiedDateTime=$this->value[0]->LastModifiedDateTime;
     ...
     */
    function getSubject() {
        //return $this->value[0]->Subject->getSummary();
   		 $arr=$this->value;
        $Subject=array();
        if(is_array($arr)){
            foreach ($arr as $id) {
    ...
    function getStartTimeUTC($user = false) {
        ...
        //$when = $this->value[0]->Start->DateTime->getStart();
        $arr=$this->value;
        $when=array();
        if(is_array($arr)){
            foreach ($arr as $id) {
                $when[]=$id->Start->DateTime->getStart();
    ...
    function getEndTimeUTC($user = false) {
       ...
       // $when = $this->value[0]->End->DateTime->getEnd();

        $arr=$this->value;
        $when=array();
        if(is_array($arr)){
            foreach ($arr as $id) {
                $when[]=$id->End->DateTime->getEnd();
    ...
    function getDescription() {
        //return $this->value[0]->BodyPreview->getDescription(); 

        $arr=$this->value;
        $BodyPreview=array();
        if(is_array($arr)){
            foreach ($arr as $id) {
                $BodyPreview[]=$id->BodyPreview->getDescription(); 
                
                //print_r($Subject);
            }
        }
        else
            $BodyPreview=$this->value[0]->BodyPreview->getDescription(); 
        return $BodyPreview;
    }
     ...
     */
    function getWhere() {
        //$where = $this->value[0]->Locations->DisplayName->getLocation();
        ///return $where;
        ...
        if(is_array($arr)){
            foreach ($arr as $id) {
                $where[]=$id->Locations->DisplayName->getLocation();
    ...
    public function getAttendees() {
        ...
        $attendees = $this->value[0]->Attendees[0]->EmailAddress->Name->getAttendees();
