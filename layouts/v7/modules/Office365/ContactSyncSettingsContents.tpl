...
{strip}    
    <form class="form-horizontal" name="contactsyncsettings">
        ...
        <input id="office365_fields" type="hidden" value='{Zend_Json::encode($OFFICE365_FIELDS)}' />
        <div class="sync-settings">
            <div class="row">
                <div class="col-sm-3 col-xs-3">
                    <h5 class="module-title pull-left">{vtranslate('LBL_SELECT_OFFICE365_GROUP_TO_SYNC',$MODULENAME)}&nbsp;</h5>
                </div>
                <div class="col-sm-4 col-xs-4">
                    <select class="select2" name="office365_group" style="width:250px;">
                        <option value="all">{vtranslate('LBL_ALL',$MODULENAME)}</option>
                        {assign var=IS_GROUP_DELETED value=1}
                        {foreach item=ENTRY from=$OFFICE365_GROUPS['entry']}
            ...
            <div class="btn-group pull-right" style = "margin:5px;">
                <button id="office365sync_addcustommapping" class="btn btn-default btn-sm dropdown-toggle" type="button" data-toggle="dropdown">
        ...
        <div id="office365syncfieldmapping" style="margin:5px;">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="hidden" class="office365_field_name" value="{$OFFICE365_FIELDS[$FLDNAME]['name']}" />
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="hidden" class="office365_field_name" value="{$OFFICE365_FIELDS['organizationname']['name']}" />
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="hidden" class="office365_field_name" value="{$OFFICE365_FIELDS['email']['name']}" />
                            {assign var="OFFICE365_TYPES" value=$OFFICE365_FIELDS[$FLDNAME]['types']}
                            <select class="select2 office365-type" style="width:200px;" data-category="email">
                                {foreach item=TYPE from=$OFFICE365_TYPES}
                                    <option value="{$TYPE}" {if $FIELD_MAPPING[{$FLDNAME}]['office365_field_type'] eq $TYPE}selected{/if}>{vtranslate('Email',$MODULENAME)} ({vtranslate($TYPE,$MODULENAME)})</option>
                                {/foreach}
                            </select>&nbsp;&nbsp;
                            <input type="text" class="office365-custom-label" style="visibility:{if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                   value="{if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] eq 'custom'}{$FIELD_MAPPING[$FLDNAME]['office365_custom_label']}{/if}" 
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="hidden" class="office365_field_name" value="{$OFFICE365_FIELDS['email']['name']}" />
                            {assign var=OFFICE365_TYPES value=$OFFICE365_FIELDS['email']['types']}
                            <select class="select2 office365-type" style="width:200px;" data-category="email">
                                {foreach item=TYPE from=$OFFICE365_TYPES}
                                    <option value="{$TYPE}" {if $FIELD_MAPPING['secondaryemail']['office365_field_type'] eq $TYPE}selected{/if}>{vtranslate('Email',$MODULENAME)} ({vtranslate($TYPE,$MODULENAME)})</option>
                                {/foreach}
                            </select>&nbsp;&nbsp;
                            <input type="text" class="office365-custom-label" style="visibility:{if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                   value="{if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] eq 'custom'}{$FIELD_MAPPING[$FLDNAME]['office365_custom_label']}{/if}" 
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="hidden" class="office365_field_name" value="{$OFFICE365_FIELDS['phone']['name']}" />
                            {assign var=OFFICE365_TYPES value=$OFFICE365_FIELDS['phone']['types']}
                            <select class="select2 stretched office365-type" style="width:200px;" data-category="phone">
                                {foreach item=TYPE from=$OFFICE365_TYPES}
                                    <option value="{$TYPE}" {if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] eq $TYPE}selected{/if}>{vtranslate('Phone',$MODULENAME)} ({vtranslate($TYPE,$MODULENAME)})</option>
                                {/foreach}
                            </select>&nbsp;&nbsp;
                            <input type="text" class="office365-custom-label" style="visibility:{if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                   value="{if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] eq 'custom'}{$FIELD_MAPPING[$FLDNAME]['office365_custom_label']}{/if}" 
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="hidden" class="office365_field_name" value="{$OFFICE365_FIELDS['address']['name']}" />
                            {assign var=OFFICE365_TYPES value=$OFFICE365_FIELDS['address']['types']}
                            <select class="select2 stretched office365-type" style="width:200px;" data-category="address">
                                {foreach item=TYPE from=$OFFICE365_TYPES}
                                    <option value="{$TYPE}" {if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] eq $TYPE}selected{/if}>{vtranslate('Address',$MODULENAME)} ({vtranslate($TYPE,$MODULENAME)})</option>
                                {/foreach}
                            </select>&nbsp;&nbsp;
                            <input type="text" class="office365-custom-label" style="visibility:{if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                   value="{if $FIELD_MAPPING[$FLDNAME]['office365_field_type'] eq 'custom'}{$FIELD_MAPPING[$FLDNAME]['office365_custom_label']}{/if}" 
                    ...
                    {foreach key=VTIGER_FIELD_NAME item=CUSTOM_FIELD_MAP from=$CUSTOM_FIELD_MAPPING}
                        <tr>
                            <td>
                                {if $CUSTOM_FIELD_MAP['office365_field_name'] eq 'gd:email'}
                                ...
                                {else if $CUSTOM_FIELD_MAP['office365_field_name'] eq 'gd:phoneNumber'}
                                ...
                                {else if $CUSTOM_FIELD_MAP['office365_field_name'] eq 'gContact:userDefinedField'}
                                ...
                                {else if $CUSTOM_FIELD_MAP['office365_field_name'] eq 'gContact:website'}
                            ...
                            <td>
                                <input type="hidden" class="office365_field_name" value="{$CUSTOM_FIELD_MAP['office365_field_name']}" />
                                {if $CUSTOM_FIELD_MAP['office365_field_name'] eq 'gd:email'}
                                    {assign var=OFFICE365_TYPES value=$OFFICE365_FIELDS['email']['types']}
                                    <select class="select2 office365-type" style="width:200px;" data-category="email">
                                        {foreach item=TYPE from=$OFFICE365_TYPES}
                                            <option value="{$TYPE}" {if $CUSTOM_FIELD_MAP['office365_field_type'] eq $TYPE}selected{/if}>{vtranslate('Email',$MODULENAME)} ({vtranslate($TYPE,$MODULENAME)})</option>
                                        {/foreach}
                                    </select>&nbsp;&nbsp;
                                    <input type="text" class="office365-custom-label" style="visibility:{if $CUSTOM_FIELD_MAP['office365_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                           value="{if $CUSTOM_FIELD_MAP['office365_field_type'] eq 'custom'}{$CUSTOM_FIELD_MAP['office365_custom_label']}{/if}" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"/>
                                {else if $CUSTOM_FIELD_MAP['office365_field_name'] eq 'gd:phoneNumber'}
                                    {assign var=OFFICE365_TYPES value=$OFFICE365_FIELDS['phone']['types']}
                                    <select class="select2 office365-type" style="width:200px;" data-category="phone">
                                        {foreach item=TYPE from=$OFFICE365_TYPES}
                                            <option value="{$TYPE}" {if $CUSTOM_FIELD_MAP['office365_field_type'] eq $TYPE}selected{/if}>{vtranslate('Phone',$MODULENAME)} ({vtranslate($TYPE,$MODULENAME)})</option>
                                        {/foreach}
                                    </select>&nbsp;&nbsp;
                                    <input type="text" class="office365-custom-label" style="visibility:{if $CUSTOM_FIELD_MAP['office365_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                           value="{if $CUSTOM_FIELD_MAP['office365_field_type'] eq 'custom'}{$CUSTOM_FIELD_MAP['office365_custom_label']}{/if}" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"/>
                                {else if $CUSTOM_FIELD_MAP['office365_field_name'] eq 'gContact:userDefinedField'}
                                    <input type="hidden" class="office365-type" value="{$CUSTOM_FIELD_MAP['office365_field_type']}">
                                    <input type="text" class="office365-custom-label" value="{$CUSTOM_FIELD_MAP['office365_custom_label']}" style="width:190px;" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"/>
                                {else if $CUSTOM_FIELD_MAP['office365_field_name'] eq 'gContact:website'}
                                    {assign var=OFFICE365_TYPES value=$OFFICE365_FIELDS['url']['types']}
                                    <select class="select2 office365-type" style="width:200px;" data-category="url">
                                        {foreach item=TYPE from=$OFFICE365_TYPES}
                                            <option value="{$TYPE}" {if $CUSTOM_FIELD_MAP['office365_field_type'] eq $TYPE}selected{/if}>{vtranslate('URL',$MODULENAME)} ({vtranslate($TYPE,$MODULENAME)})</option>
                                        {/foreach}
                                    </select>&nbsp;&nbsp;
                                    <input type="text" class="office365-custom-label" style="visibility:{if $CUSTOM_FIELD_MAP['office365_field_type'] neq 'custom'}hidden{else}visible{/if};width:190px;" 
                                           value="{if $CUSTOM_FIELD_MAP['office365_field_type'] eq 'custom'}{$CUSTOM_FIELD_MAP['office365_custom_label']}{/if}" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"/>
