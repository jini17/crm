...
{strip}
	...
	{foreach key=RECORDID item=RECORD_MODEL from=$TASKS}
		...
		<div class="entries ui-draggable">
			...
			<div class="task clearfix" data-recordid="{$RECORD_MODEL->get('id')}" data-status="{strtolower($STATUSVAL|replace:' ':'_')}" data-basicinfo='{json_encode($RECORD_BASIC_INFO)}' style="border-left:4px solid {$COLORS[$STATUSVAL]}">
