            ...
            <div class="btn-toolbar row-fluid">
                <div class="span6">
                    ...
                    {foreach item=RELATED_LINK from=$RELATED_LIST_LINKS['LISTVIEWBASIC']}
                        ...
                        data-url="{$RELATED_LINK->getUrl()}&hidebutton=true"
                    {/if}
...
<div class="relatedContents contents-bottomscroll">
    <div class="bottomscroll-div">
		...
		{assign var=ATTACHMENTS value=$RELATED_RECORD->getFileDetails()}
	
                <tr class="listViewEntries" data-id={$RELATED_RECORD->getId()}>
                    {foreach item=HEADER_FIELD from=$RELATED_HEADERS}
                        {assign var=RELATED_HEADERNAME value=$HEADER_FIELD->get('name')}
                        <td class="{$WIDTHTYPE}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap><a href="?module=Documents&action=DownloadFile&record={$RELATED_RECORD->getId()}&fileid={$ATTACHMENTS['attachmentsid']}">
                            {if $HEADER_FIELD->isNameField() eq true or $HEADER_FIELD->get('uitype') eq '4'}
                                {$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}
                            {elseif $RELATED_HEADERNAME eq 'access_count'} Jitu2
                            ...
                            </a></td><td nowrap class="{$WIDTHTYPE}">
					...
					{if $POPUP neq 'Leave'}                                       
					 <a href="{$RELATED_RECORD->getFullDetailViewUrl()}"><i title="{vtranslate('LBL_SHOW_COMPLETE_DETAILS', $MODULE)}" class="icon-th-list alignMiddle"></i></a>&nbsp;
                                        ...
                                        {if $IS_EDITABLE}
                                            ...
                                            {else}
					{/if}  
                                              {if $POPUP neq 'Leave'}  <a href='{$RELATED_RECORD->getEditViewUrl()}'><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="icon-pencil alignMiddle"></i></a>{/if}
                                            {/if}
                                        {/if}
                                        {if $IS_DELETABLE}
                                            {if $RELATED_MODULE_NAME eq 'Calendar'}
