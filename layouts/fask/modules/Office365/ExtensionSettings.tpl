...
<div class="col-sm-12 col-xs-12 extensionContents" style="position: relative;">
        ...
        <form name="settingsForm" action="index.php" method="POST" >
                        ...
                        <div class="col-sm-12 col-xs-12">
                                <table class="listview-table table-bordered" align="center">
                                        ...
                                        <tbody>
                                                <tr>
                                                        <td>
                                                                <select name="Contacts[office365_group]" class="inputElement select2 row" style="min-width: 250px;">
                                                                        <option value="all">{vtranslate('LBL_ALL',$MODULENAME)}</option>
                                                                        {assign var=IS_GROUP_DELETED value=1}
                                                                        {foreach item=ENTRY from=$OFFICE365_CONTACTS_GROUPS['entry']}
                                                        ...
                                                        <td>
                                                                <select name="Contacts[sync_direction]" class="inputElement select2 row" style="min-width: 250px;">
                                                                        ...
                                                                        <option value="10" {if $CONTACTS_SYNC_DIRECTION eq 10} selected {/if}> {vtranslate('LBL_SYNC_FROM_OFFICE365_TO_VTIGER', $MODULE)} </option>
                                                                        <option value="01" {if $CONTACTS_SYNC_DIRECTION eq 01} selected {/if}> {vtranslate('LBL_SYNC_FROM_VTIGER_TO_OFFICE365', $MODULE)} </option>
                                                                </select>
                                                        </td>
                                                </tr>
                                                <tr>
                                                        <td>
                                                                <select name="Calendar[office365_group]" class="inputElement select2 row" style="min-width: 250px;">
                                                                        {if count($OFFICE365_CALENDARS) eq 0}
                                                                                <option value="primary">{vtranslate('LBL_PRIMARY',$MODULENAME)}</option>
                                                                        {/if}
                                                                        {foreach item=CALENDAR_ITEM from=$OFFICE365_CALENDARS}
                                                                                <option value="{if $CALENDAR_ITEM['primary'] eq 1}primary{else}{$CALENDAR_ITEM['id']}{/if}" {if $SELECTED_OFFICE365_CALENDAR eq $CALENDAR_ITEM['id']}selected{/if} {if $SELECTED_OFFICE365_CALENDAR eq 'primary' && $CALENDAR_ITEM['primary'] eq 1} selected {/if}>{$CALENDAR_ITEM['summary']}</option>
                                                                        {/foreach}
                                                                </select>
                                                        </td>
                                                        ...
                                                        <td>
                                                                <select name="Calendar[sync_direction]" class="inputElement select2 row" style="min-width: 250px;">
                                                                        ...
                                                                        <option value="10" {if $CALENDAR_SYNC_DIRECTION eq 10} selected {/if}> {vtranslate('LBL_SYNC_FROM_OFFICE365_TO_VTIGER', $MODULE)} </option>
                                                                        <option value="01" {if $CALENDAR_SYNC_DIRECTION eq 01} selected {/if}> {vtranslate('LBL_SYNC_FROM_VTIGER_TO_OFFICE365', $MODULE)} </option>
                                                                </select>
                                                        </td>
                                                </tr>
                                        </tbody>
                ...
                {if !$IS_SYNC_READY}
                        <div class="row">
                                <div class="col-sm-12 col-xs-12">
                                        <h3 class="module-title pull-left"> {vtranslate('LBL_OFFICE365_CONNECT_MSG', $MODULE)} </h3>
                                </div>
                        </div>
                        <br>
                        <div class="row">
                ...
                {else}
                        <div class="row">
                                <div class="col-sm-12 col-xs-12">
                                        <h3 class="module-title pull-left"> {vtranslate('LBL_OFFICE365_ACCOUNT_DETAILS', $MODULE)} </h3>
                                </div>
                        </div>
                        <br>
                        {if $USER_EMAIL}
                                <div class="row">
                                        <div class="col-sm-3 col-xs-3">
                                                <h5 class="module-title pull-left fieldLabel"> {vtranslate('LBL_OFFICE365_ACCOUNT_SYNCED_WITH', $MODULE)} </h5>
                                        </div>
                        ...
                        <div class="col-sm-12 col-xs-12">
                                <div class="vt-default-callout vt-info-callout"> 
                                        ...
                                        <div>
                                                ...
                                                {vtranslate('LBL_REDIRECT_URL', $MODULE)} : <span style="color: #15c !important">{Office365_Config_Connector::getRedirectUrl()}</span>
                                        </div>
                                        <br>
                                        <img src="modules/Office365/images/redirect_uri.png" />
                                </div>
                        </div>
                </div>
