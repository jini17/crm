...
function fillInAddresspickup() {
    ...
    for (var i = 0; i < place.address_components.length; i++) {
        ...
        if (addressType == 'administrative_area_level_1') {
            ...
            document.getElementById('Invoice_editView_fieldName_bill_state').value = val;
        }
        /************* Country ***************************/
        if (addressType == 'country') {
            var val = place.address_components[i][componentForm[addressType]];
            document.getElementById('Invoice_editView_fieldName_bill_country').value = val;
        }
        /*********** City**************/
...
function fillInAddressdispatch() {
    ...
    for (var i = 0; i < place.address_components.length; i++) {
        ...
        if (addressType == 'country') {
            var val = place.address_components[i][componentForm[addressType]];
            document.getElementById('Invoice_editView_fieldName_ship_country').value = val;
        }
        /*********** City**************/
        if(addressType == 'locality'){
            var val = place.address_components[i][componentForm[addressType]];
