...
<div class="modelContainer">
	<div class="modal-header">
		...
		<h3>{vtranslate('LBL_DELETE', $QUALIFIED_MODULE)} - {$RECORD_MODEL->getChildName()}</h3>
	</div>
	<form class="form-horizontal" id="treeDeleteForm" method="post" action="index.php">
		<input type="hidden" name="module" value="{$MODULE}" />
		<input type="hidden" name="parent" value="Settings" />
		<input type="hidden" name="action" value="Delete" />
		<input type="hidden" name="parenttree" id="parenttree" value="{$RECORD_MODEL->getId()}" />
		<input type="hidden" name="regionid" id="regionid" value="{$RECORD_MODEL->getRegionid()}" />

		<div class="modal-body">
			<h5>{vtranslate('LBL_TRANSFER_OWNERSHIP',$QUALIFIED_MODULE)}</h5>
			<div class="control-group">
				<div class="control-label"><span class="redColor">*</span>{vtranslate('LBL_TO_OTHER_REGION',$QUALIFIED_MODULE)}</div>
				<div class="controls">
					<input id="transfer_record" name="transfer_record" type="hidden" value="" class="sourceField">
					<div class="input-prepend input-append">
						<span class="add-on cursorPointer" id="clearRegion">
