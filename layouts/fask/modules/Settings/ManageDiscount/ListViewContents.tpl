...
<div class="row-fluid">
	<span class="span8 btn-toolbar">
		<button class="btn btn-success addButton" onclick="Settings_ManageDiscount_List_Js.triggerEditEvent('module=ManageDiscount&parent=Settings&view=EditAjax&mode=new');">
			<i class="icon-plus"></i>
			<strong>{vtranslate('LBL_ADDDISCOUNT',$QUALIFIED_MODULE)}</strong>
		</button>
		<button class="btn pull-left btn-danger" onclick="Settings_ManageDiscount_List_Js.triggerDeleteEvent('module=ManageDiscount&parent=Settings&view=DeleteAjax');">
			<i class="icon-minus"></i>
			<strong>{vtranslate('LBL_DELETEDISCOUNT',$QUALIFIED_MODULE)}</strong>
		</button>
	</span>
</div>
			...
			<td width="1%" nowrap class="{$WIDTHTYPE}">
				...
				{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
					...
					<td class="listViewEntryValue {$WIDTHTYPE}"  width="{$WIDTH}%" nowrap>
						&nbsp;{if $LISTVIEW_HEADERNAME eq 'discount_status'}
								{if $LISTVIEW_ENTRY->getDisplayValue($LISTVIEW_HEADERNAME) eq 0}
									{vtranslate('LBL_ACTIVE',$QUALIFIED_MODULE)}
								{else}
									{vtranslate('LBL_INACTIVE',$QUALIFIED_MODULE)}
								{/if}
							{else if $LISTVIEW_HEADERNAME eq 'discount_level'}
								{if $LISTVIEW_ENTRY->getDisplayValue($LISTVIEW_HEADERNAME) eq 'I'}
									{vtranslate('LBL_INDIVIDUAL',$QUALIFIED_MODULE)}
								{else if $LISTVIEW_ENTRY->getDisplayValue($LISTVIEW_HEADERNAME) eq 'G'}
									{vtranslate('LBL_GROUP',$QUALIFIED_MODULE)}
								{else} 										{vtranslate('LBL_BOTH',$QUALIFIED_MODULE)}
								{/if}
							{else}
								...
								<div class="pull-right actions">
									<span class="actionImages">
										{foreach item=RECORD_LINK from=$LISTVIEW_ENTRY->getRecordLinks()}
											...
											<a onclick="{$RECORD_LINK_URL|substr:strlen("javascript:")};">
