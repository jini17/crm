...
{assign var=PICKLIST_VALUES value=$FIELD_MODEL->getRegions()}
...
<select id="{$MODULE}_{$smarty.request.view}_fieldName_{$FIELD_MODEL->get('name')}" {if $MODULE eq 'SalesTarget'}multiple{/if} class="select2" {if $MODULE eq 'SalesTarget'} name="{$FIELD_MODEL->getFieldName()}[]" {else} name="{$FIELD_MODEL->getFieldName()}" {/if} data-fieldinfo='{$FIELD_INFO}' {if $FIELD_MODEL->isMandatory() eq true} data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} {/if} style="width: 60%">
	{if $MODULE eq 'SalesTarget'}	
	<option value="all" {if $FIELD_MODEL->get('fieldvalue') eq 'all'}selected{/if}>{vtranslate('LBL_ALL', $MODULE)}</option>
	{/if}	
