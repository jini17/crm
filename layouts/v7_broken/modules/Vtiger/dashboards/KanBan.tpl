            ...
            <div class='datacontent'>
                ...
                <div class="data-body row">
                    ...
                    {foreach item=STATUS from=$STATUSES}
                        <div class="col-lg-4 contentsBlock {strtolower($STATUS|replace:' ':'_')} ui-droppable" data-status='{$STATUS}' data-page="{$PAGE}">
                            <div class="{strtolower($STATUS|replace:' ':'_')}-header" style="border-bottom: 2px solid ">
                                <div class="title" style="background:{$COLORS[$STATUS]}"><span>{$STATUS}</span></div>
                            </div>
                            <br>
                            <div class="{strtolower($STATUS)}-content content" data-status='{$STATUS|replace:' ':'_'}' style="border-bottom: 1px solid {$COLORS[$STATUS]};padding-bottom: 10px">
                                {if $USER_PRIVILEGES_MODEL->hasModuleActionPermission($MODULE_MODEL->getId(), 'CreateView')}
                                    <div class="input-group">
                                        <input type="text" class="form-control taskSubject {$STATUS|replace:' ':'_'}" placeholder="{vtranslate('LBL_ADD_TASK_AND_PRESS_ENTER', $MODULE)}" aria-describedby="basic-addon1" style="width: 99%">
                                        <span class="quickTask input-group-addon js-task-popover-container more cursorPointer" id="basic-addon1" style="border: 1px solid #ddd; padding: 0 13px;">
											<a href="#" id="taskPopover" status='{$STATUS}'><i class="fa fa-plus icon"></i></a>
										</span>
                                    </div>
                                {/if}
                                <br>
                                <div class='{strtolower($STATUS)}-entries container-fluid scrollable dataEntries padding20' style="height:400px;overflow:auto;width:400px;padding-left: 0px;padding-right: 0px;">
