...
class EnhancedQueryGenerator extends QueryGenerator
{
            ...
            foreach ($valueSqlList as $valueSql) {
                if (in_array($baseFieldName, $this->referenceFieldList)) {
                    ...
                    if ($fieldName == 'birthday' && !$this->isRelativeSearchOperators($conditionInfo['operator'])) {
                        ...
                        /* Added By Khaled*/
                    } elseif ($fieldName == 'date_joined' &&  $conditionInfo['operator'] == 'thisweek') {
                        $fieldSql .= "YEARWEEK(date_joined)=YEARWEEK(NOW())";
                        /* Added By Khaled*/
                    } elseif ($fieldName == 'date_joined' &&  $conditionInfo['operator'] == 'thismonth') {
                        $fieldSql .= "DATE_FORMAT(date_joined,'%m')=".date('m');
    ...
    static function getSpecialDateConditionQuery($comparator, $date)
    {
        switch ($comparator) {
            case 'bw' :
                return " BETWEEN '$date[0]' AND '$date[1]' ";
