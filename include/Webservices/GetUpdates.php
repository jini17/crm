...
require_once 'include/Webservices/DescribeObject.php';
	...
	// Mabruk
	function vtws_sync_office365Contacts($mtime,$elementType,$syncType,$user){ 
		global $adb, $recordString,$modifiedTimeString;
		...
		foreach($accessableModules as $elementType){
			...
			$fromClause .= " INNER JOIN (select modifiedtime, crmid,deleted,setype FROM $baseCRMTable WHERE setype=?";
			if(!$applicationSync){
				$fromClause.= 'and smownerid IN('.generateQuestionMarks($ownerIds).')';
				$params = array_merge($params,$ownerIds);
			}
