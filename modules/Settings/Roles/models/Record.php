...
//	}
	...
	public function save() {
		...
		if($mode == 'edit') {
			$sql = 'UPDATE vtiger_role SET planid=?, rolename=?, parentrole=?, depth=?, allowassignedrecordsto=? WHERE roleid=?';
			$params = array($planid, $this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'), $roleId);
			$db->pquery($sql, $params);
		} else {
			$sql = 'INSERT INTO vtiger_role(planid, roleid, rolename, parentrole, depth, allowassignedrecordsto) VALUES (?,?,?,?,?,?)';
			$params = array($planid, $roleId, $this->getName(), $this->getParentRoleString(), $this->getDepth(), $this->get('allowassignedrecordsto'));
			$db->pquery($sql, $params);
