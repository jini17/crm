...
class Settings_Profiles_Record_Model extends Settings_Vtiger_Record_Model {
	...
	public function save() {
		...
		if(!$profileId) {
			...
			$sql = 'INSERT INTO vtiger_profile(planid,profileid, profilename, description, directly_related_to_role) VALUES (?,?,?,?,?)';
			$params = array($plan, $profileId, $profileName, $description, $isProfileDirectlyRelatedToRole);
			$isNewProfile = true;
		} else {
			$sql = 'UPDATE vtiger_profile SET planid=?, profilename=?, description=?, directly_related_to_role=? WHERE profileid=?';
			$params = array($plan,$profileName, $description, $isProfileDirectlyRelatedToRole, $profileId);
    ...
    public static function getInstanceByName($profileName , $checkOnlyDirectlyRelated=false, $excludedRecordId = array()) {
     ...
     public static function isAllowedModule($module, $plantitle) {
        $db = PearDatabase::getInstance();
        //$db->setDebug(true);
	...
	public function getPlan(){
		...
		$result = $db->pquery("SELECT plantitle from secondcrm_plan WHERE planid=?",array($planid));

		return $db->query_result($result,0,'plantitle');
	}
}
