...
class Settings_LayoutEditor_Module_Model extends Vtiger_Module_Model {
	...
	public function addField($fieldType, $blockId, $params) {
		...
		//modified by jitu@CustomField as Standard field on 27 Dec 2017
        $name  = str_replace(' ','_', strtolower($params['fieldName']));  
        if($this->checkFIeldExists($label)){
            throw new Exception(vtranslate('LBL_DUPLICATE_FIELD_LABEL_EXISTS', 'Settings::LayoutEditor'), 513);
        }

        if($this->checkFIeldNameExists($name)){
            throw new Exception(vtranslate('LBL_DUPLICATE_FIELD_NAME_EXISTS', 'Settings::LayoutEditor'), 514);
        } 
        //End here
		...
		//modified by jitu@CustomField for create field user define
		    $columnName = str_replace(' ','_', strtolower($params['fieldName']));//'cf_'.$max_fieldid;
        //end here
		...
		$fieldModel->set('name', $columnName)
				   ...
				   ->set('iscustom', 1);  //added by jitu@CustomField for adding custom Field
	...
	public function checkFIeldNameExists($fieldName) {
        ...
        if($this->getName() == 'Calendar' || $this->getName() == 'Events') {
            //Check for field exists in both calendar and events module
            $tabId = array('9','16');
        }
