...
//		}
	...
	//Added By Mabruk on 06/04/2018 for SMS Vendor Slection	
	function getActiveSMSvendors(){
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT id,providertype FROM vtiger_smsnotifier_servers WHERE isactive = 1",array());
		$count = $db->num_rows($result);
		$vendors = array();
		for ($i = 0; $i < $count; $i++){
			$vendors[$i]['id'] = $db->query_result($result,$i,'id');
			$vendors[$i]['providertype'] = $db->query_result($result,$i,'providertype');
		}
		return $vendors;
	}
	//End

	//Added By Mabruk on 06/04/2018 for SMS Template Slection	
	function getSMSTemplates(){
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT templateid,templatename FROM vtiger_emailtemplates WHERE templatetype = 'SMS'",array());
		$count = $db->num_rows($result);
		$templates = array();
		for ($i = 0; $i < $count; $i++){
			$templates[$i]['templatename'] = $db->query_result($result,$i,'templatename');
			$templates[$i]['templateid'] = $db->query_result($result,$i,'templateid');
		}
		return $templates;
	}
	//End	
