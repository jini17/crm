...
class QuotesPDFContentViewer extends Vtiger_PDF_InventoryContentViewer {
	...
	function calculateWidth($totalcolumsArray) {
		...
		switch($totalcolums) {
			...
			case 1 :
				 $widthArray = array(0,0,540,0,0,0,0,0,0,0);
			break;

			case 2 :
				if($pdfsettings['sno']==1) {
					$widthArray = array(40,0,520,0,0,0,0,0,60);
					
				} else { 
					if($pdfsettings['order']==1) {
						$widthArray = array(0,80,480,0,0,0,0,0,60);
					} else {
						$widthArray = array(0,0,420,60,60,60,60,60,60);
					}
				}
			break;
			...
			case 3 :
				if($pdfsettings['sno']==1) {
					if($pdfsettings['order']==1) {
						$widthArray = array(30,80,440,0,0,0,0,0,0);
					} else {
						$widthArray = array(30,0,440,60,60,60,60,60,60);
					}
					
				} else { 
					if($pdfsettings['order']==1) {
						$widthArray =  array(0,60,440,60,60,60,60,60,60);
					} else {
						$widthArray =  array(0,0,360,60,60,60,60,60,60);
					}
				}
			break;
			case 4 :
				...
				if($pdfsettings['sno']==1) {
					if($pdfsettings['order']==1) {
						$widthArray = array(20,60,400,60,60,60,60,60,60);
					} else {
						$widthArray = array(20,0,400,60,60,60,60,60,60);
					}
					
				} else { 
					if($pdfsettings['order']==1) {
						$widthArray =  array(0,60,400,60,60,60,60,60,60);
					} else {
						$widthArray =  array(0,0,400,60,60,60,60,60,60);
					}
				}
			break;
			case 5 :
				if($pdfsettings['sno']==1) {
					if($pdfsettings['order']==1) {
						$widthArray = array(30,60,300,60,60,70,70,70,60);
					} else {
						$widthArray = array(30,0,300,60,60,70,70,70,60);
					}
					
				} else { 
					if($pdfsettings['order']==1) {
						$widthArray =  array(0,60,300,60,60,60,60,70,70);
					} else {
						$widthArray =  array(0,0,300,60,60,60,60,70,70);
					}
				}
			...
			case 6 :
				...
				if($pdfsettings['sno']==1) {
					if($pdfsettings['order']==1) {
						$widthArray = array(20,60,280,60,60,60,60,60,60);
					} else {
						$widthArray = array(20,0,280,60,60,60,60,60,60);
					}
					
				} else { 
					if($pdfsettings['order']==1) {
						$widthArray =  array(0,60,280,60,60,60,60,60,60);
					} else {
						$widthArray =  array(0,0,280,60,60,60,60,60,60);
					}
				}
			...
			case 7 :
				if($pdfsettings['sno']==1) {
					if($pdfsettings['order']==1) {
						$widthArray = array(20,60,200,60, 60,60,60,60,60);
					} else {
						$widthArray = array(20,0,200,60,60, 60,60,60,60);
					}
					
				} else { 
					if($pdfsettings['order']==1) {
						$widthArray =  array(0,60,260,60,60,60,60,60,60);
					} else {
						$widthArray =  array(0,0,200,60,60, 60,60,60,60);
					}
				}
			...
			case 8 :
				...
				if($pdfsettings['sno']==1) {
					if($pdfsettings['order']==1) {
						$widthArray = array(20,60,160,60,60,60,60,60,60);
					} else {
						$widthArray = array(20,0,160,60,60,60,60,60,60);
					}
					
				} else { 
					if($pdfsettings['order']==1) {
						$widthArray =  array(0,60,60,60,60,60,60,60,60);
					} else {
						$widthArray =  array(0,0,60,60, 60,60,60,60,60);
					}
				}
			break;
			case 9 :
				...
				if($pdfsettings['sno']==1) {
					if($pdfsettings['order']==1) {
						$widthArray = array(20,60,100,60,60,60,60,60,60,60);
					} else {
						$widthArray = array(20,0,100,65,60,60,60,60,60,60);
					}
					
				} else { 
					if($pdfsettings['order']==1) {
						$widthArray =  array(0,60,100,60,60,60,60,60,60,60);
					} else {
						$widthArray =  array(0,0,100,60, 60,60,60,60,60,60,60);
					}
				}
			break;
	...
	function getLineItemCols($mode,$labelModels,$pdfsettings) {
		...
		if($mode == 'individual') {
			...
			if($pdfsettings['showindunit'] ==1) {
				$cols[$i]['label'] = $labelModels->get('UsageUnit'); 
				$cols[$i]['align'] = 'left';				
			...
			if($pdfsettings['showgroupunit'] ==1) {
				$cols[$i]['label'] = $labelModels->get('UsageUnit'); 
				$cols[$i]['align'] = 'left';				
				$cols[$i]['width'] = $calculatewidth[4]; 				
				$i++;
	...
	function getNoCols($mode, $pdfsettings) {
		...
		if($mode == 'individual') { 
			...
			if($pdfsettings['showindunit']==1) { $retcols++; $colsettings['unit'] = 1;}
			...
			if($pdfsettings['showgroupunit']==1) { $retcols++; $colsettings['unit'] = 1;}
	...
	function getLineItemVals($mode, $pdfsettings,$models) {
		...
		if($mode == 'individual') {
			...
			for ($index = 0; $index < $totalModels; $index++) {
				...
				if($pdfsettings['showindunit'] ==1) {
					$colValues[$index][$i]['value'] = $model->get('UsageUnit'); 	
					$colValues[$index][$i]['align'] = 'left';
				...
				if($pdfsettings['showindamount'] ==1) {
					...
					$colValues[$index][$i]['align'] = 'right';	
					$colValues[$index][$i]['width'] = $calculatewidth[8]; 				
					$i++;
				}
			}
	...
	function display($parent) {
		...
		if($pdfsettings['showsummary']==1) {
			 $h2 = $pdf->getStringHeight($headersummary['title'].'<br />'.implode('<br />',$headersummary['dates']),60);
		}
								...
								foreach($discounts as $discount) { 
								   if($discount['discount_amount']!=0) {
								   	if($discount['postdiscount_criteria']=='P') {
										$valstr .= "<i style='font-size:11pt;'>".$desc['discount_title']." (".$discount['postdiscount_value']."%) : ".$discount['discount_amount']."</i><br />";
									} else {
		...
		$summaryhtml = '<!--subtotal start-->
			<table cellpadding="5"  border="0">
				<tr><td width="25pt"></td><td width="260pt">'.$description.'</td>';
			...
			foreach($summaryCellKeys as $key) {
				...
				if(!empty($keyvalue) && $keyvalue !='0.00') {
					...
					if($key == 'Discount') {
						$summaryhtml .= '<tr><td colspan="3" width="190pt"  bgcolor='.$bgcolortotal.'><strong>'.$key.':</strong></td>	
						...
						foreach($summarydiscounts as $kindex=>$val) {
							$summaryhtml .= '<tr><td colspan="3" width="190pt"  bgcolor='.$bgcolortotal.'><i>'.$kindex.':</i></td>	
			...
			for($step=0;$step<$pages;$step++) {
				...
				if($step+1 > 1) {
					$pdf->setPage($step+1);
						...
						if($pdfsettings['showorgaddress'] ==1) {
							$companyaddress = "<br /><br /><br /><span style=\"font-size:15pt\">".$headerinfo['summary']."</span><br />".$headerinfo[content]."<br />".$vatlabel." :  ".$headerinfo['vatid'];
							$ymargin = $h1==0?$y:$y+15;			
