...
include_once dirname(__FILE__). '/QuotesPDFHeaderViewer.php';
include_once dirname(__FILE__). '/QuotesPDFContentViewer.php';
...
class Vtiger_QuotePDFController extends Vtiger_InventoryPDFController{
	...
	function getContentViewer() {
			$contentViewer = new QuotesPDFContentViewer();
	...
	function buildContentModels() { 
		...
		foreach($associated_products as $productLineItem) { 
			...
			$contentModel->set('UsageUnit', $usageunit);		//added by jitu on request from IXt by Hafizah
	...
	function buildContentLabelModel() {
		...
		$labelModel->set('UsageUnit',  getTranslatedString('Unit',$this->moduleName));
	...
	function buildSummaryModel() {
		...
		$summaryModel->set(getTranslatedString("Net Total", $this->moduleName), $netTotal);
