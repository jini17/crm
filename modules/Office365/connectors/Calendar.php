...
vimport('~~/libraries/office365-api-php-client/src/Google/Client.php');
vimport('~~/libraries/office365-api-php-client/src/Google/Service/Calendar.php');
...
Class Office365_Calendar_Connector extends WSAPP_TargetConnector {
    ...
    public function getCalendar($SyncState, $user = false) {
        ...
        if (count($calendarRecords) > 0) {
            $maxModifiedTime = date('Y-m-d H:i:s', strtotime(Office365_Contacts_Model::vtigerFormat(end($calendarRecords)->getUpdated())) + 1);
        }
    ...
    public function pullCalendars($list=false) { 
        $calendarList = $this->service->calendarList->listCalendarList(); echo "sadad";die;
