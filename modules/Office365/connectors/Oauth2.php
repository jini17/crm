...
class Office365_Oauth2_Connector {
    ...
    protected $scopes = array(
        'Contacts' => 'https://outlook.office.com/contacts.readwrite offline_access',
        'Calendar' => 'https://outlook.office.com/calendars.readwrite offline_access',
    );
    
    public $token;

    const OAUTH2_AUTH_URL = 'https://login.microsoftonline.com/common/oauth2/v2.0/authorize';
    
    const OAUTH2_TOKEN_URI = 'https://login.microsoftonline.com/common/oauth2/v2.0/token';
    
    //const OAUTH2_Autoh = 'https://accounts.google.com/o/oauth2/revoke';
    public function __construct($module,$userId=false) {
		...
		$this->client_id = Office365_Config_Connector::$clientId;
		$this->client_secret = Office365_Config_Connector::$clientSecret;
		$this->redirect_uri = Office365_Config_Connector::getRedirectUrl();
    ...
    public function getState($source) {
        global $site_URL;
        $callbackUri = rtrim($site_URL, '/') . '/index.php?module=Office365&view=List&operation=sync&sourcemodule='.$this->source_module . '&service=' . $source;
    ...
    protected function retreiveToken() {
        ...
        $decodedAccessToken =json_decode($data[0]);
   ...
   public function authorize() {
        ...
        if($this->hasStoredToken()) {
           ...
           //var_dump($token);die;
            ...
            if($_REQUEST['service'] && $_SESSION['code']) {
