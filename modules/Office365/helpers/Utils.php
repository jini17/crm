...
class Office365_Utils_Helper {
    ...
    const fieldmapping_table_name = 'vtiger_office365_sync_fieldmapping';
    ...
    public static function updateSyncTime($sourceModule, $modifiedTime = false, $user = false) {
        ...
        if (!self::getSyncTime($sourceModule, $user)) {
            if ($modifiedTime) {    
                $db->pquery('INSERT INTO vtiger_office365_sync (office365module,user,synctime,lastsynctime) VALUES (?,?,?,?)', array($sourceModule, $user->id, $modifiedTime, date('Y-m-d H:i:s')));
            }
        } else {
    ...
    private static function intialiseUpdateSchema() {
        if (!Vtiger_Utils::CheckTable('vtiger_office365_sync')) {
            Vtiger_Utils::CreateTable('vtiger_office365_sync', '(office365module varchar(50),user int(10), synctime datetime,lastsynctime datetime)', true);
        }
    }
    ...
    public static function getLastSyncTime($sourceModule) {
        ...
        self::intialiseUpdateSchema();
        $user = Users_Record_Model::getCurrentUserModel();
        $result = $db->pquery('SELECT lastsynctime FROM vtiger_office365_sync WHERE user=? AND office365module=?', array($user->id, $sourceModule));
        if ($result && $db->num_rows($result) > 0) {
            $row = $db->fetch_array($result);
    ...
    /**
     * To get users currently in sync with Office365
     * @global type $adb
     * @return type
     */
    public static function getSyncUsers(){
        global $adb;
        $users = array();
        $result = $adb->pquery("SELECT DISTINCT userid FROM vtiger_office365_oauth2", array());
    ...
    static function saveSettings($request) {
        ...
        $office365_group = $request->get('office365_group');
        ...
        if(Office365_Utils_Helper::hasSettingsForUser($userId,$source_module)) {
            ...
            $params = array($office365_group,$sync_direction,$userId,$source_module);
            $db->pquery($sql, $params);
        } else {
            $sql = 'INSERT INTO ' . self::settings_table_name . '(user,module,clientgroup,direction) VALUES (?,?,?,?)';
            $params = array($userId,$source_module,$office365_group,$sync_direction);
            $db->pquery($sql, $params);
        }
    }
    ...
    static function saveFieldMappings($sourceModule, $fieldMappings) {
        ...
        $sql = 'INSERT INTO ' . self::fieldmapping_table_name . ' (vtiger_field,office365_field,office365_field_type,office365_custom_label,user) VALUES ';
    ...
    static function getSelectedContactGroupForUser($user = false) {
        ...
        $userId = $user->getId();
        if(!Office365_Utils_Helper::hasSettingsForUser($userId,'Contacts')) {
    ...
    static function getSyncDirectionForUser($user = false, $module = 'Contacts') {
        ...
        if(!Office365_Utils_Helper::hasSettingsForUser($user->getId(),$module)) {
            ...
            $query_result=$db->query_result($result, 0, 'direction');
            return $query_result;
        }
    }
    ...
    static function getFieldMappingForUser($user = false) {
        ...
        $fieldmapping = array(
            'salutationtype' => array(
                'office365_field_name' => 'DisplayName',
                'office365_field_type' => '',
                'office365_custom_label' => ''
            ),
            'firstname' => array(
                'office365_field_name' => 'GivenName',
                'office365_field_type' => '',
                'office365_custom_label' => ''
            ),
            'lastname' => array(
                'office365_field_name' => 'Surname',
                'office365_field_type' => '',
                'office365_custom_label' => ''
            ),
            'title' => array(
                'office365_field_name' => 'JobTitle',
                'office365_field_type' => '',
                'office365_custom_label' => ''
            ),            
            'birthday' => array(
                'office365_field_name' => 'Birthday',
                'office365_field_type' => '',
                'office365_custom_label' => ''
            ),
            'email' => array(
                'office365_field_name' => 'ImAddresses',
                'office365_field_type' => 'work',
                'office365_custom_label' => ''
            ),
            'phone' => array(
                'office365_field_name' => 'MobilePhone1',
                'office365_field_type' => 'work',
                'office365_custom_label' => ''
            ),
            'mailingaddress' => array(
                'office365_field_name' => 'OfficeLocation',
                'office365_field_type' => 'work',
                'office365_custom_label' => ''
            ),
        ...
        $sql = 'SELECT vtiger_field,office365_field,office365_field_type,office365_custom_label FROM ' . self::fieldmapping_table_name . ' WHERE user = ?';
        $result = $db->pquery($sql,array($user->getId()));
        for($i=0;$i<$db->num_rows($result);$i++) {  
            $row = $db->fetch_row($result);
            $fieldmapping[$row['vtiger_field']] = array(
                'office365_field_name' => $row['office365_field'],
                'office365_field_type' => $row['office365_field_type'],
                'office365_custom_label' => $row['office365_custom_label']
            );
        }
        return $fieldmapping;
    }
    ...
    static function toOffice365Xml($string) {
    ...
    static function saveSyncSettings($request) {
        ...
        if(Office365_Utils_Helper::hasSettingsForUser($userId,$source_module)) {
            ...
            $params = array($office365_group,$sync_direction,$enabled,$userId,$source_module);
            $db->pquery($sql, $params);
        } else {
            $sql = 'INSERT INTO ' . self::settings_table_name . ' VALUES (?,?,?,?,?)';
            $params = array($userId,$source_module,$office365_group,$sync_direction,$enabled);
            $db->pquery($sql, $params);
        }
    }
