...
class Office365_Calendar_Model extends WSAPP_SyncRecordModel {
    ...
    function getStartTimeUTC($user = false) {
        ...
        $when = $this->data['entity']['Start'];
        if (empty($when)) {
            $gStart = "00:00";
        } else if ($when['DateTime']) {
            $gStart = $when['DateTime'];
        } else {
            $gStart = "00:00";
        }
    ...
    function getVisibility($user) {
        $visibility = $this->data['entity']['Sensitivity'];
        if(strpos($visibility, 'Private') !== false)
            return 'Private';
        else if(strpos($visibility, 'Normal') !== false)
            return 'Public';
        else {
     ...
     */
    function getWhere() {
        $where = $this->data['entity']['Location']['DisplayName'];
        return $where;
    }
    ...
    public function getAttendees() {
        ...
        $attendees = $this->data['entity']['Attendees'];
        if(is_array($attendees)) {
            foreach($attendees as $attendee) {
                $attendeeDetails[] = $attendee['EmailAddress']['Address'];
            }
        }
