...
LIMIT 3"; 
        ...
        public function saveEducationDetail($request)
        {
              ...
              // $db->setDebug(true);
                ...
                $edu_type  	= decode_html($request['edu_type']);
                ...
                $resultmajorcheck  = $db->pquery("SELECT major_id FROM secondcrm_studyarea WHERE major = ?",array($majorTxt));
                       ...
                       $education = CRMEntity::getInstance('Education');
                        //$education = new Education();
                       

                if(!empty($eduId)) {
                        $education->retrieve_entity_info($eduId, 'Education');
                        //update Education
                        $education->mode = 'edit';
                        $education->id = $eduId;
                        $return = 1;
        ...
        function get_education_types(){
           ...
           // $db->setDebug(true);
            ...
            if($num_rows > 0){
                ...
                $data[$i] = $db->query_result($query, $i,'education_type');
                }
            }
            return $data;
        }
