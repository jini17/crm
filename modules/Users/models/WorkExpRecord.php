    ...
    public static function getInstance($id=null) {
        ...
        public function getWorkExpDetail($uw_id) {
                ...
                $sql = "SELECT tblSCW.uw_id, tblSCW.company_id,tblSCC.company_title, 
                                ...
                                tblSCW.currentlyworking, 	tblSCW.job_type, 
                ...
                $workexpDetail['job_type'] = $db->query_result($result, 0, 'job_type');
        ...
        public function getUserWorkExpList($userId) {
                ...
                if($db->num_rows($result) > 0) {
                        for($i=0;$i<$db->num_rows($result);$i++) {
                                ...
                                $userWorkExpList[$i]['job_type'] = $db->query_result($result, $i, 'job_type');
        ...
        public function saveworkexpDetail($request)
        {
                ...
                $locTxt 	= decode_html($request['locatxt']);
                                            $jobtype 	= decode_html($request['job_type']);
                ...
                if(!empty($uwId)) {
                        $params = array($comId,$desId,$jobtype,$locId,$startDate,$endDate,$jobDesc,$isview,$isworking,$uwId);			
                        $result = $db->pquery("UPDATE secondcrm_userworkexp SET company_id = ?,designation_id=?,job_type=?,location_id=?, start_date = ?, end_date=?,description = ?, isview=?, currentlyworking=? WHERE 	uw_id=?",array($params));
                        $return = 1;	

                } else {
                        $params = array($userid, $comId,$jobtype,$desId,$locId,$startDate,$endDate,$jobDesc,$isview, $isworking);
                        $result = $db->pquery("INSERT INTO secondcrm_userworkexp SET user_id = ?, company_id = ?,job_type=?,designation_id=?,location_id=?, start_date = ?, end_date=?,description = ?, isview=?, currentlyworking=?", array($params));
                        $return = 0;
                }	
                return $return;
        }
