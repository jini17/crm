					...
					foreach($modelColumn2 as $label => $value) { 
						if(is_array($value)) {
							foreach($value as $l=>$v) {
								$text = $l;	
								$textvalue = $v;	
								//add by amir for execute .00 in total amount
...
EOF;
			...
			if (isset($modelColumnCenter['Purchase Order']) && isset($modelColumnCenter['Sales Order'])) {
				unset($modelColumnCenter['Purchase Order'], $modelColumnCenter['Sales Order']);
			}
			...
			foreach($modelColumnCenter as $label => $value) { 
				if(!empty($value)) {
					...
					$html .= "
						<table width=\"540pt\">
							<tr>
							<td width=\"65pt\"><font color=\"#333\"><strong>{$label}</strong></font></td>
