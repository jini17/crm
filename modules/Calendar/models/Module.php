...
class Calendar_Module_Model extends Vtiger_Module_Model {
	...
	*/
     ...
     * Modified by Nirbhay for KanBan Chart
	 */
	...
	public function getAllTasksbyPriority($conditions = false, $pagingModel) {
		...
		$mandatoryFields = array("id","taskstatus","parent_id","contact_id");
		...
		//Commented by Nirbhay to remove more button
		//$startIndex = $pagingModel->getStartIndex();
		//$pageLimit = $pagingModel->getPageLimit();
        /*Nirbhay comment ended*/
		...
		for($i=0;$i<$noOfRows;$i++){
			...
			$status = strtolower(str_replace(" ","_",$model->get('status')));



			if($status){
				$tasks[$status][$model->getId()] = $model;
			}
		}

		if(count($tasks[$status]) > $pageLimit){
			//array_pop($tasks[$status]);
