...
class Calendar_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	public function process(Vtiger_Request $request) {
	  //echo "<pre>"; print_r($request->get('taskstatus'));

        /***Added By Nirbhay to replace the underscore with space***/

        $statusvalues = $request->get('taskstatus');
        if(stripos($statusvalues, "_")>0){
            //echo "Inside the correct condition";
            $statusvalues = str_replace("_", " ", $statusvalues);
           // echo "value after the replace".$statusvalues;
            $request->set('taskstatus',$statusvalues);
        }
