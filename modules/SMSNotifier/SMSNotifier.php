...
class SMSNotifier extends SMSNotifierBase {
	...
	static public $defaultProvider = false;
	...
	static function setDefaultProvider($provider){
...
class SMSNotifierManager {
	...
	static function getActiveProviderInstance() {
		...
		$defaultprovider = SMSNotifier::getProvider();
		$where = '';
		$limit  = 'LIMIT 1';
		if($defaultprovider){
			$limit  = '';
			$where = " AND providertype='$defaultprovider'";
		}
		global $adb;
		$result = $adb->pquery("SELECT * FROM vtiger_smsnotifier_servers WHERE isactive = 1 $where $limit", array());
		if($result && $adb->num_rows($result)) {
