...
class Documents extends CRMEntity {
	...
	function save_module($module)
	{
		...
		if($this->column_fields[$filetype_fieldname] == 'I' || $this->column_fields[$filetype_fieldname] == 'G'){
		...
		//Added By Mabruk for Goolge Drive Intergration 27/03/2018
		if ($this->column_fields[$filetype_fieldname] == 'G')
				$filelocationtype = 'G';
	...
	function insertIntoAttachment($id,$module)
	{
		...
		foreach($_FILES as $fileindex => $files)
		{
			if($files['name'] != '' && $files['size'] > 0)
			{
				...
				$file_saved = $this->uploadAndSaveFile($id,$module,$files,'Attachment',$this->column_fields['filelocationtype']);
			}
		}
