...
//$db->setDebug(true);
                ...
                if($filter == 'passport'){
                        $query = "SELECT passportvisaid as empcid, concat(vtiger_users.first_name, '', vtiger_users.last_name) as fullname, vtiger_users.department, vtiger_users.title as designation, DAY(date_expired) as expirydate_day, MONTH(date_expired) as expirydate_month, YEAR(date_expired) as expirydate_year FROM vtiger_passportvisa
                                          INNER JOIN vtiger_users ON vtiger_users.id = vtiger_passportvisa.employee_id 
                                        WHERE  date_expired >= CURDATE() OR date_expired <= CURDATE() - INTERVAL 30 DAY LIMIT ?, ?";
                }
                if($filter == 'visa'){
                        $query = "SELECT passportvisaid as empcid,concat(vtiger_users.first_name, '', vtiger_users.last_name) as fullname, vtiger_users.department, vtiger_users.title as designation, DAY(visa_date_expired) as expirydate_day, MONTH(visa_date_expired) as expirydate_month, YEAR(visa_date_expired) as expirydate_year FROM vtiger_passportvisa
                                          INNER JOIN vtiger_users ON vtiger_users.id = vtiger_passportvisa.employee_id 
                                          WHERE visa_date_expired >= CURDATE() OR visa_date_expired <= CURDATE() - INTERVAL 30 DAY  LIMIT ?, ?";
                }
