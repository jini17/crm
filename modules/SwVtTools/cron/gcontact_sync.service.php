...
$sql = 'SELECT * FROM vtiger_google_sync WHERE googlemodule = "Contacts"';
$result = $adb->query($sql);

$oldCurrentUser2 = vglobal('current_user');

$enableSharedContacts = false;
...
echo 'GoogleContacts Sync'.PHP_EOL;
    ...
    while($user = $adb->fetchByAssoc($result)) {
        try {
            ...
            $controller = new Google_Contacts_Controller($user2);

            if($enableSharedContacts === true && method_exists($controller, 'getContactId')) {
                $calId = $controller->getContactId();
