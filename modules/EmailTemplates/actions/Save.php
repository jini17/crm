...
class EmailTemplates_Save_Action extends Vtiger_Save_Action {
	...
	public function checkPermission(Vtiger_Request $request) {
		...
		if (!Users_Privileges_Model::isPermitted($moduleName, 'Save', $record) || !$currentUser->isAdminUser()) {
			throw new AppException(vtranslate('LBL_PERMISSION_DENIED'));
		}
	}
	...
	public function process(Vtiger_Request $request) {
		...
		$recordModel->set('templateType', $request->get('templateType')); // Added By Mabruk for Notification Template on 30/04/2018
